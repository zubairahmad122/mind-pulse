...
// ─── Circular Sound Menu (Centered Around Orb) ───────────────────
function CenterSoundMenu({
  selectedId,
  onSelect,
  onStop,
  accentColor,
  ...
  const CENTER_Y = 280; // Positioned lower to be at bottom of orb

  return (
    <View style={styles.soundMenuContainer}>
      ...
      {BREATHING_MUSIC.map((music, idx) => {
        ...
        return (
          <Text
            key={`label-${music.id}`}
            style={[
              styles.soundButtonLabel,
              {
                left: x - 20,
                top: y,
                color: selectedId === music.id ? music.color : 'rgba(255,255,255,0.3)',
              },
            ]}
          >
            {music.label}
...
// ─── Main Component ────────────────────────────────────────────
export const CenterSessionControls = memo(function CenterSessionControls({
  voiceVolume,
  ambientVolume,
  onVoiceVolumeChange,
...
const styles = StyleSheet.create({
  ...
  // Center sound menu (overlaps orb at bottom)
  soundMenuContainer: {
    position: 'absolute',
    top: 0,
    left: 0,
